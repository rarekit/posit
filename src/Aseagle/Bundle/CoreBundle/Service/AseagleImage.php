...
class AseagleImage {
    ...
    public function ftpSave($ftp, $filename = null, $remotePath=null, $remoteFile=null, $quality = null) {
        // Determine quality, filename, and format
        $quality = $quality ? : $this->quality;
        $filename = $filename ? : $this->filename;
        $format = $this->file_ext($filename) ? : $this->original_info['format'];
        ...
        if (!$ftp->isDir($remotePath)) {
            $ftp->mkDirRecursive($remotePath);
        }
        $ftp->put($remotePath.$remoteFile, $filename, FTP_BINARY);
