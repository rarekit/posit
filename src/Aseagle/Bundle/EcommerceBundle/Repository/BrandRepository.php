...
class BrandRepository extends EntityRepository
{
    ...
    public function search($criteria, $orderBy, $limit, $offset, $count = false)
    {
        ...
        foreach ($criteria as $key => $value) {
            if (is_numeric($value)) {
                ...
                if ($key == 'categories') {
                    $query->innerJoin('o.categories', 'c');
                    $query->$where("c.id IN (:$key)")->setParameter($key, implode(",", $value));
                } else {
                    $query->$where("o.$key IN (:$key)")->setParameter($key, implode(",", $value));
                }
    ...
    public function getBrandByCategories($catIds)
    {
        $query = $this->createQueryBuilder('o')->select('o.id, o.picture, o.name, o.slug, c.id as cid');
        $query->innerJoin('o.categories', 'c');
