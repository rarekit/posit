<?php

namespace Aseagle\Bundle\EcommerceBundle\Repository;

use Doctrine\ORM\EntityRepository;

/**
 * ProductRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class ProductRepository extends EntityRepository {

    /**
     *
     * @param array $criteria            
     * @param string $orderBy            
     * @param string $limit            
     * @param string $offset            
     * @return Ambigous <\Doctrine\ORM\mixed, \Aseagle\Bundle\EcommerceBundle\Repository\multitype:>
     */
    public function getList($criteria, $orderBy = null, $limit = null, $offset = null) {
        return $this->search($criteria, $orderBy, $limit, $offset);
    }

    /**
     *
     * @param array $criteria            
     * @return Ambigous <\Doctrine\ORM\mixed, \Aseagle\Bundle\EcommerceBundle\Repository\multitype:>
     */
    public function getTotal($criteria) {
        return $this->search($criteria, array (), null, null, true);
    }

    /**
     *
     * @param array $criteria            
     * @param array $orderBy            
     * @param integer $limit            
     * @param integer $offset            
     * @param string $count            
     * @return \Doctrine\ORM\mixed|multitype:
     */
    public function search($criteria, $orderBy, $limit, $offset, $count = false) {
        $query = $this->createQueryBuilder('o');
        if ($count) {
            $query->select('COUNT(o.id)');
        }
        
        $where = 'where';
        foreach ($criteria as $key => $value) {
            if (is_numeric($value)) {
                switch ($key) {
                    case 'price_from' :
                        $query->$where("o.price >= :$key")->setParameter($key, $value);
                        break;
                    case 'price_to' :
                        $query->$where("o.price <= :$key")->setParameter($key, $value);
                        break;
                    case 'quantity_from' :
                        $query->$where("o.quantity >= :$key")->setParameter($key, $value);
                        break;
                    case 'quantity_to' :
                        $query->$where("o.quantity <= :$key")->setParameter($key, $value);
                        break;
                    default :
                        $query->$where("o.$key = :$key")->setParameter($key, $value);
                }                
            } elseif ($value instanceof \DateTime) {
                switch ($key) {
                    case 'created_from' :
                        $query->$where("o.created >= :$key")->setParameter($key, $value->format('Y-m-d') . ' 00:00:00');
                        break;
                    case 'created_to' :
                        $query->$where("o.created <= :$key")->setParameter($key, $value->format('Y-m-d') . ' 23:59:59');
                        break;
                    default :
                        $query->$where("o.$key = :$key")->setParameter($key, $value->format('Y-m-d'));
                }
            } elseif (is_string($value)) {
                
                $query->$where("o.$key LIKE :$key")->setParameter($key, "%$value%");
            } elseif (is_array($value)) {
                
                $query->$where("o.$key IN (:$key)")->setParameter($key, $value);
            } elseif (is_object($value)) {
                
                $query->$where("o.$key = :$key")->setParameter($key, $value->getId());
            } else {
                if ($value == null && $key == 'parent') {
                    $query->$where("o.$key is NULL");
                } elseif ($value != null) {
                    $query->$where("o.$key = :$key")->setParameter($key, $value);
                }
            }
            $where = 'andWhere';
        }        
        
        if (! empty($orderBy)) {
            foreach ($orderBy as $key => $value) {
                $keyOrder [] = "o.$key";
                $sortValue = $value;
            }
            $query->orderBy(implode(',', $keyOrder), $sortValue);
        }
        
        if ($limit) {
            $query->setMaxResults((int) $limit);
            $query->setFirstResult((int) $offset);
        }
        
        if ($count) {
            return $query->getQuery()->getSingleScalarResult();
        } else {
            return $query->getQuery()->getResult();
        }
    }

    /**
     * @param unknown $brandId
     * @param unknown $orderBy
     * @param unknown $limit
     * @param unknown $offset
     * @param string $count
     * @return \Doctrine\ORM\mixed|multitype:
     */
    public function getProductByBrand($brandId, $orderBy, $limit, $offset, $count = false) {
        $query = $this->createQueryBuilder('o');
        if ($count) {
            $query->select('COUNT(o.id)');
        }
        
        $query->where('o.brand = :brand')->setParameter(':brand', $brandId);
        
        if (! empty($orderBy)) {
            foreach ($orderBy as $key => $value) {
                $keyOrder [] = "o.$key";
                $sortValue = $value;
            }
            $query->orderBy(implode(',', $keyOrder), $sortValue);
        }
        
        if ($limit) {
            $query->setMaxResults((int) $limit);
            $query->setFirstResult((int) $offset);
        }
        
        if ($count) {
            return $query->getQuery()->getSingleScalarResult();
        } else {
            return $query->getQuery()->getResult();
        }
    }
    
    /**
     * @param unknown $brandId
     * @param unknown $orderBy
     * @param unknown $limit
     * @param unknown $offset
     * @param string $count
     * @return \Doctrine\ORM\mixed|multitype:
     */
    public function getProductByCategory($catId, $orderBy, $limit, $offset, $count = false) {
        $query = $this->createQueryBuilder('o');
        if ($count) {
            $query->select('COUNT(o.id)');
        }
        $query->leftJoin('o.categories', 'c');
        $query->where('c.id = :category')->setParameter(':category', $catId);
    
        if (! empty($orderBy)) {
            foreach ($orderBy as $key => $value) {
                $keyOrder [] = "o.$key";
                $sortValue = $value;
            }
            $query->orderBy(implode(',', $keyOrder), $sortValue);
        }
    
        if ($limit) {
            $query->setMaxResults((int) $limit);
            $query->setFirstResult((int) $offset);
        }
    
        if ($count) {
            return $query->getQuery()->getSingleScalarResult();
        } else {
            return $query->getQuery()->getResult();
        }
    }
    
    
    public function getProductOnHomepage() {
        $query = $this->createQueryBuilder('o')   
            ->select('o.id, o.name, o.thumbnail, o.slug, c.id as cid')    
            ->join('o.categories', 'c')
            ->where('o.enabled = 1 AND o.showHomepage = 1')
            ->orderBy('o.created', 'DESC');
        
        return $query->getQuery()->getResult();
    }
    
    public function searchByKeyword($keyword, $limit, $offset, $count = false) {
        $query = $this->createQueryBuilder('o');
        if ($count) {
            $query->select('COUNT(o.id)');
        } else {
            $query->select('o.id')->distinct()->select('o.id, o.name, o.price, o.thumbnail, o.slug');
        }
        $query->leftJoin('o.categories', 'c')
            ->leftJoin('o.brand', 'b')            
            ->where('o.name LIKE :keyword')
            ->orWhere('c.title LIKE :keyword')
            ->orWhere('b.name LIKE :keyword')
            ->setParameter(':keyword', "%$keyword%")
            ->orderBy('o.created', 'DESC');
        
        if ($limit) {
            $query->setMaxResults((int) $limit);
            $query->setFirstResult((int) $offset);
        }
        
        if ($count) {
            return $query->getQuery()->getSingleScalarResult();
        } else {
            return $query->getQuery()->getResult();
        }
    }

}


